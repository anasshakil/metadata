...
/**
 * @param {string | import("node:fs").ReadStream} file If you're directly passing a network stream make sure to set the disable_cache option to true, otherwise the temporary file will be deleted after reading the metadata.
...
export async function getMetadata(file, options = {}) {
    ...
    try {
        ...
        if (typeof file === "object") {
            del = !(disable_cache === false);
            tempFile = await _streamFile(file);
        } else if (_isURL(file)) {
            del = disable_cache === true;
 ...
 * @param {ExifMetadataWriteOptions} [options] 
 */

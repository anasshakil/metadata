 ...
 * @property {string} [type] **[Advanced]**</br>[Default: type of `value`]. The type of the metadata tag. This property is optional and will be used to determine the type of tag for auto generated tags.
 *
 ...
 * @property {boolean} [all] [Default: `false`]. If `true`, all the metadata tags will be returned. This will override the `tags` option. **NOTE**: This option can cause significant performance issues. Use it only if you need all the metadata tags.
 *
 * @ {boolean} [auto] [Default: `false`]. If `true`, the custom metadata tags will be automatically added to the configuration file.</br>**NOTE**: This option can cause performance issues.
 *
 *
 ...
 * @property {string} path [Default _`exiftool`_]. The path of the ExifTool binaries.
 ...
 * @property {boolean} [stream] [Default: `false`]. If `true`, instead of returning metadata JSON, an instance of readable stream containing metadata string will be returned. **NOTE**: This option will return raw metadata instead of JSON, custom parser will be required to parse JSON. JSON.parse() will always throw an error.
 *
 *
...
export async function __ext__(file, options = {}) {
    ...
    try {
        ...
        const result = await _cmdExec(files, {
            exiftool_path: exif_path,
            configPath: config,
            args: _allMd ? ["-ee3", "-api", "RequestAll=3"] : _args,
            fast_mode: fast,
            _ret_raw_stream_: _raw_stream_,
 ...
 * @param {ExifMetadataWriteOptions & ExifMetadataReadWriteOptions & _ExperimentalFeatures_} options
 * @returns {Promise<{ message?: string, file?: string[], metadata?: { pre?: any[], new?: any[] } }>}
 */
export async function __w__(file, options = {}) {
    ...
    try {
        ...
        const {
            ...
            fast,
            stream: _raw_stream_,
            auto: auto_generate_tags,
        } = options;
        if (!(tags.length > 0)) {
            throw new Error("Provide at least one tag to update the metadata");
        }
        const { files, cache: __cache } = await __linear_path(file, {
            ...options,
            batch: { write: {} },
        });
        _cache = __cache;
        ...
        // FIXME: Exiftool requires very specific tag name and type for Tag groups related to the file.
        // TODO: Check file mime type than implement tag group casting supported by Exiftool
        if (auto_generate_tags) {
            // Auto tags generator implementation
        }
        const _args = _argsWriteRaw(
            delete_all ? [...tags, { name: "all" }] : tags
        );
        const result = await _cmdExec(files, {
            exiftool_path: exif_path,
            configPath: config,
 ...
 * @param {ExifCopyMetadataOptions & ExifMetadataReadWriteOptions & _ExperimentalFeatures_} options
 */
export async function __c__(src, dst, options = {}) {
    ...
    try {
        ...
        if (Array.isArray(src)) {
            throw new Error(
                "Source file can only be a single file or directory"
            );
        }
        const {
            files: _srcFiles,
        ...
        const { files: _dstFiles, cache: __dst_cache } = await __linear_path(
            dst,
            { ..._dstOp, batch: { write: {} } }
        );
        _dCache = __dst_cache;
...
async function __linear_path(file, options = {}) {
    ...
    if (!file) {
        throw new Error(
            "Invalid file path or unreadable stream passed as an argument"
        );
    }
    let tempFile = file;
    let _del = false;
    ...
    return {
        ...
        del: _del,
        del_batch: __del_batch,
        cache: Array.isArray(tempFile)
            ? tempFile.filter((_f) => _f.includes(".tmp"))
            : tempFile.includes(".tmp")
            ? [tempFile]
            : [],
    };
}
...
async function __batch__(files, options = {}) {
    try {
        ...
        const { no_network_cache, no_stream_cache = [] } = _readOptions;
        const __del = {};
        const _files =
            (
                await Promise.all(
                    files.map((_f) => {
                        ...
                        if (typeof _f === "object") {
                            ...
                            __del[__temp] = !(
                                no_stream_cache === false && __fName?.length > 0
                            );
...
function __cp_data(metadata = {}) {
    if (
        metadata.src === null &&
        metadata.dst.pre === null &&
        metadata.dst.new === null
    )
        return null;
    if (metadata.dst.pre === null && metadata.dst.new === null)
        return { metadata: metadata.src };
    if (
        metadata.src !== null &&
        metadata.dst.pre !== null &&
        metadata.dst.new !== null

...
export async function _exiftoolConfigurator(options = {}) {
    try {
        ...
        if (!no_cache_cleanup) { await __rmTempFiles(); }
        if (tags.length < 1) {
...
async function _saveConfig(config) {
    try {
        ...
        await fs.writeFile(path.join(__dir__(), "bin/.ExifTool_config"), `#${_hash}\n${config}`);
    } catch (e) {
        throw e;
    }
...
async function _getHash() {
    try {
        const readable = createReadStream(path.join(__dir__(), "bin/.ExifTool_config"));
...
export async function __rmTempFiles() {
    try {
        await fs.rm(path.join(__dir__(), ".tmp"), { recursive: true, force: true });

...
const { __undefinedTag, _clr_read_buffer_ } = __utils__;
...
export function _cmdExec(_fpath, options = {}) {
    try {
        return new Promise(async (resolve, reject) => {
            ...
            if (_ret_raw_stream_ === true) { return cps.stdout; }
            cps.stdout.on("data", function (data) {
                ...
                if (fast_mode) {
                    ...
                    if (__index !== -1) {
                        ...
                        // cps.stdout.unshift(null)
                    }
                }
                metadata += chunk;
                // console.log(cps.stdout._readableState)
                if (done) {
                    _clr_read_buffer_(cps.stdout);

...
/**
 * ExifTool CLI
 * @param {string} command Exiftool commands for further reading visit: https://exiftool.org/exiftool_pod.html
...
function __cli__(command, callback, options = {}) {
    ...
    if (!_isProcessActive) {
        __raw_process__ = spawn(
            "perl",
            _config_stay_open_(exif_path, true, false, config, true)
        );
    ...
    command += " -execute0\n";
    __raw_process__.stdin.write(command.split(" ").join("\n"), "utf8");

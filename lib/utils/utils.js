 ...
 */
function __clr_process_internal_events__(o) {
...
function __readable_stream_parser__(o, buf, fast_mode) {
    ...
    if (done) {
        __clr_process_internal_events__(o);
    }
...
function __check_update__() {
    return new Promise((resolve, reject) => {
        let _release_ = "";
        get(
            "https://sourceforge.net/projects/exiftool/best_release.json",
            (res) => {
                res.on("data", (d) => {
                    _release_ += d.toString();
                });
                res.on("error", (e) => {
                    reject(e);
                });
                res.on("end", () => {
                    if (res.statusCode >= 200 && res.statusCode < 300) {
                        try {
                            _release_ = JSON.parse(_release_);
                            if (!_release_?.release)
                                _release_ = { error: "Update failed!" };
                        } catch (e) {
                            _release_ = { error: "Update failed!" };
                        }
                    } else {
                    ...
                    resolve(_release_?.release);
                });
            }
        );
    });

    async function _download_update_() {
        return;
        const _remote_version = await __check_update__();
        if (!_remote_version?.url) throw _remote_version;
        const _local_version = await fs.readFile(
            path.join(__dir__(), ".config_files/.version_tracker"),
            { encoding: "utf8" }
        );
        if (_remote_version === _local_version) {
            return { message: "No update available" };
        }
        //NOTE: Node.js doesn't support built-in zip files.
        // PLAN: No future plans for auto updating Exiftool.
        get(_remote_version?.url, (res) => {
            res.on("data", (d) => {
...
function _tag_group_type_cast_(f0, val) {
    return "";
}
